<?php
$public_redet_as = new Redet_As_0001_Public('redet-as-0001', '1.0.0');
$activities = $public_redet_as->get_activities_redet_as();
/* $tipo = "V";
$cedula = 1000001;
$prospectos = $public_redet_as->buscar_cedula_rif_ra($tipo, $cedula); */
                    ...
                    <div class="dashboard-content-block activities-list-wrap">
                        <ul class="list-unstyled activities-list">
                            ...
                            foreach( $activities['data']['results'] as $activity ) 
                            {
                                ...
                                $estatus_actividad_redet_as = $activity->estatus_actividad_redet_as;

                                if (in_array($type, $actividades_ra, true)) 
                                {
                                    if ($estatus_actividad_redet_as == 'Abierta')
                                    {
                                        $notificacion_redet_as = isset($meta['notificacion']) ? $meta['notificacion'] : '';
                                        
                                        if (isset($meta['enlace_prospectos']))
                                        {
                                            $enlace_prospectos_redet_as = $meta['enlace_prospectos'];
                                        } 
                                        else
                                        {
                                            $enlace_prospectos_redet_as = '';
                                        }

                                        if (isset($meta['id_prospectos']))
                                        {                                 
                                            $id_prospectos_redet_as = $meta['id_prospectos'];
                                        }
                                        else
                                        {
                                        ...
                                        {  
                                            $id_prospectos_serialize_redet_as = serialize($id_prospectos_redet_as);
                                            $id_prospectos_urlencode_redet_as = urlencode($id_prospectos_serialize_redet_as);
        
                                            $enlace_prospectos_redet_as .= '&id_prospectos_redet_as=' . $id_prospectos_urlencode_redet_as;
                                            ?>
                                            <li class="activitiy-item d-flex">
                                                ...
                                                <div class="activitiy-item-body">
                                                    ...
                                                    <?php
                                                    echo '<p>';
                                                        echo wp_kses(__( '<strong>' . esc_attr($notificacion_redet_as) . '</strong>', 'houzez' ), $allowed_html_array);
                                                    echo '</p>';
                                                        
                                                    if(!empty($enlace_prospectos_redet_as)) 
                                                    {           
                                                        echo '<p>';
                                                            echo '<a href="'.esc_url($enlace_prospectos_redet_as).'"><i 
                                                            class="houzez-icon icon-arrow-button-circle-right mr-2"></i> <strong>'.esc_html__('View', 'houzez').'</strong></a>';
                                                        echo '</p>';
                                                    }
                                                    ?>
                                                </div>
